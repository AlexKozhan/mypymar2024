"""
This module contains tests for the library system
including the Book and User classes.
It uses pytest for testing and logging to capture
log messages.
"""
...
def test_reserve_book(setup_library, caplog):
    ...
    assert ("Attempting to reserve book 'White Fang' "
            "by user Alex.") in caplog.text
    assert ("Book 'White Fang' is reserved "
            "by user Alex.") in caplog.text
...
def test_take_book(setup_library, caplog):
    ...
    assert ("Attempting to take book 'White Fang' "
            "by user Alex.") in caplog.text
    assert ("Book 'White Fang' is taken "
            "by user Alex.") in caplog.text
...
def test_return_book(setup_library, caplog):
    ...
    assert ("Attempting to return book 'White Fang' "
            "by user Alex.") in caplog.text
    assert ("Book 'White Fang' is returned "
            "by user Alex.") in caplog.text
